...
type Tutor = { id: string; username: string; full_name: string; school_email: string; role: string; subjects: string[]; active: boolean; must_change_password: boolean };
...
export default function ManageTutors() {
  ...
  async function createTutor(event: FormEvent) {
    ...
    try { const response = await fetch("/api/admin/tutors", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); const result = await response.json() as { error?: string }; if (!response.ok) throw new Error(result.error); setMessage(`${form.fullName} can now sign in with @${form.username} and their temporary password.`); setForm({ fullName: "", username: "", schoolEmail: "", role: "tutor", temporaryPassword: "", subjects: [] }); await loadTutors(); }
    ...
    <main className="manage-shell">
      ...
      <section className="manage-grid">
        <form className="create-tutor-card" onSubmit={createTutor}><span className="auth-icon"><Plus /></span><h2>Add an approved tutor</h2>
          ...
          <div className="field-pair"><label>Username<Input placeholder="jordan.lee" value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} /></label><label>Access level<Select value={form.role} onValueChange={(role) => setForm({ ...form, role })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="tutor">Tutor</SelectItem><SelectItem value="officer">Officer</SelectItem><SelectItem value="admin">Administrator</SelectItem></SelectContent></Select></label></div>
