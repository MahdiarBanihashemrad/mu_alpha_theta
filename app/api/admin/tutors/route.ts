...
export async function GET() {
  ...
  const { data, error } = await createSupabaseAdminClient().from("profiles").select("id,username,full_name,school_email,login_email,role,subjects,active,must_change_password,created_at").order("full_name");
...
export async function POST(request: Request) {
  ...
  const { error: profileError } = await admin.from("profiles").insert({
    ...
    login_email: loginEmail, role, subjects: selectedSubjects, active: true, must_change_password: true,
  });
  if (profileError) {
...
export async function PATCH(request: Request) {
  ...
  const admin = createSupabaseAdminClient();
  const { data: current, error: loadError } = await admin.from("profiles").select("id,username,full_name,school_email,login_email,role,subjects,active").eq("id", id).maybeSingle();
  ...
  const update = {
    username, full_name: fullName, school_email: schoolEmail, role, subjects: selectedSubjects, active,
