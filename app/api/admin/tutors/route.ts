...
export async function GET() {
  ...
  const { data, error } = await createSupabaseAdminClient().from("profiles").select("id,username,full_name,school_email,role,subjects,active,must_change_password,created_at").order("full_name");
...
export async function POST(request: Request) {
  ...
  if (!/^[a-z0-9][a-z0-9._-]{2,39}$/.test(username) || !fullName || !/^\S+@\S+\.\S+$/.test(schoolEmail) || !roles.has(role)) return Response.json({ error: "Use a unique username with letters, numbers, dots, underscores, or hyphens." }, { status: 400 });
  ...
  const { error: profileError } = await admin.from("profiles").insert({
    id: created.user.id, username, full_name: fullName, school_email: schoolEmail,
